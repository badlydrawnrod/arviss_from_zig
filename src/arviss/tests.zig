...
test "fnmsub.s" { // 'F' extension.
    ...
    _ = ArvissExecute(&cpu, encodeRs3(rs3) | (0b00 << 25) | encodeRs2(rs2) | encodeRs1(rs1) | encodeRm(rm) | encodeRd(rd) | opcodeAsU32(Opcode.opNMSUB));
...
test "fcvt.wu.s" { // 'F' extension.
    ...
    const rd: u32 = 15;
    const rs1: u32 = 13;
    const op: u32 = 0b00001;
    const rm: u32 = @enumToInt(arviss.ArvissRoundingMode.rmDYN);
    cpu.freg[rs1] = -12345678.910; // Actually -12345679.0f because of rounding.

    const expected: u32 = asU32(@intCast(i32, -12345679));
...
test "feq.s" { // 'F' extension.
    ...
    _ = ArvissExecute(&cpu, (0b1010000 << 25) | encodeRs2(rs2) | encodeRs1(rs1) | (0b010 << 12) | encodeRd(rd) | opcodeAsU32(Opcode.opOPFP));
...
test "flt.s" { // 'F' extension.
    ...
    _ = ArvissExecute(&cpu, (0b1010000 << 25) | encodeRs2(rs2) | encodeRs1(rs1) | (0b001 << 12) | encodeRd(rd) | opcodeAsU32(Opcode.opOPFP));
...
test "fcvt.s.wu" { // 'F' extension.
    ...
    const expected: f32 = @intToFloat(f32, cpu.xreg[rs1]);
...
test "fmv.w.x" { // 'F' extension.
    ...
    const expected: f32 = @bitCast(f32, cpu.xreg[rs1]);

    _ = ArvissExecute(&cpu, (0b1111000 << 25) | encodeRs2(0b00000) | encodeRs1(rs1) | (0b000 << 12) | encodeRd(rd) | opcodeAsU32(Opcode.opOPFP));
